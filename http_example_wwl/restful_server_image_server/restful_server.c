...
static void get_snap_face(struct mg_connection *nc, struct http_message *hm) {
  /* print message*/ 
  printf("hm->body.len : %ld\n", hm->body.len);
  printf("hm->method.p : %s\nhm->method.len : %ld\n", hm->method.p, hm->method.len);
  printf("hm->uri.p : %s\nhm->uri.len : %ld\n", hm->uri.p, hm->uri.len);
  /* Send headers */ 
  mg_printf(nc, "%s", "HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\n\r\n");
...
static void ev_handler(struct mg_connection *nc, int ev, void *ev_data) {
  ...
  switch (ev) {
    case MG_EV_HTTP_REQUEST:
      ...
      else if(0 == mg_vcmp(&hm->uri, "/api/v1/snap_face")) {
        printf("hahahaha\n");
        get_snap_face(nc, hm);
      }
      else {
